...
void spiCsHigh(int pin) { digitalWrite(pin, HIGH); }
void spiCsLow(int pin) { digitalWrite(pin, LOW); }
void spiCsOutputMode(int pin) { pinMode(pin, OUTPUT); }

void spiRelease(void) { close(fb); }
void delayMs(unsigned int s) { sleep(s / 1000.0); }
void delayUs(unsigned int us) { usleep(us); }

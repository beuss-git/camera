...
struct CameraInfo CameraInfo_5MP = {
    .cameraId = "5MP",
    .supportResolution =
        RESOLUTION_320x320 | RESOLUTION_128x128 | RESOLUTION_96x96 |
        RESOLUTION_320X240 | RESOLUTION_640X480 | RESOLUTION_1280X720 |
        RESOLUTION_1600X1200 | RESOLUTION_1920X1080 | RESOLUTION_2592X1944,
    .supportSpecialEffects =
        SPECIAL_BLUEISH | SPECIAL_REDISH | SPECIAL_BW | SPECIAL_SEPIA |
        SPECIAL_NEGATIVE |
...
void cameraGetSensorConfig(ArducamCamera *camera) {
  ...
  // cameraDefaultInfo[cameraIdx]->cameraDefaultFormat;
  // camera->currentPictureMode =
  // cameraDefaultInfo[cameraIdx]->cameraDefaultResolution;
}
...
CamStatus cameraBegin(ArducamCamera *camera) {
  ...
  camera->verDateAndNumber[1] =
      readReg(camera, CAM_REG_MONTH_ID) & 0x0F; // month
  waitI2cIdle(camera);
  camera->verDateAndNumber[2] = readReg(camera, CAM_REG_DAY_ID) & 0x1F; // day
  waitI2cIdle(camera);
  camera->verDateAndNumber[3] =
      readReg(camera, CAM_REG_FPGA_VERSION_NUMBER) & 0xFF; // day
  waitI2cIdle(camera);

  writeReg(camera, CAM_REG_DEBUG_DEVICE_ADDRESS,
           camera->myCameraInfo.deviceAddress);
  waitI2cIdle(camera);
...
CamStatus cameraTakePicture(ArducamCamera *camera, CAM_IMAGE_MODE mode,
...
CamStatus cameratakeMultiPictures(ArducamCamera *camera, CAM_IMAGE_MODE mode,
...
void cameraRegisterCallback(ArducamCamera *camera, BUFFER_CALLBACK function,
                            uint8_t size, STOP_HANDLE handle) {
...
CamStatus cameraSetAutoWhiteBalanceMode(ArducamCamera *camera,
...
CamStatus cameraSetAbsoluteExposure(ArducamCamera *camera,
                                    uint32_t exposure_time) {
  // set exposure output [19:16]
  writeReg(camera, CAM_REG_MANUAL_EXPOSURE_BIT_19_16,
           (uint8_t)((exposure_time >> 16) & 0xff));
  waitI2cIdle(camera);
  // set exposure output [15:8]
  writeReg(camera, CAM_REG_MANUAL_EXPOSURE_BIT_15_8,
           (uint8_t)((exposure_time >> 8) & 0xff));
  waitI2cIdle(camera);
  // set exposure output [7:0]
  writeReg(camera, CAM_REG_MANUAL_EXPOSURE_BIT_7_0,
           (uint8_t)(exposure_time & 0xff));
  waitI2cIdle(camera);
  return CAM_ERR_SUCCESS;
}
...
CamStatus cameraSetSaturation(ArducamCamera *camera,
...
CamStatus cameraSetBrightness(ArducamCamera *camera,
...
void cameraWaitI2cIdle(ArducamCamera *camera) {
  while ((readReg(camera, CAM_REG_SENSOR_STATE) & 0X03) !=
...
uint8_t cameraHeartBeat(ArducamCamera *camera) {
  return (readReg(camera, CAM_REG_SENSOR_STATE) & 0X03) ==
         CAM_REG_SENSOR_STATE_IDLE;
}
...
CamStatus takeMultiPictures(ArducamCamera *camera, CAM_IMAGE_MODE mode,
                            CAM_IMAGE_PIX_FMT pixel_format, uint8_t num) {
  return camera->arducamCameraOp->takeMultiPictures(camera, mode, pixel_format,
                                                    num);
}

...
struct CameraInfo OV5640_CameraInfo = {
    .cameraId = "5MP",
    .supportResolution = RESOLUTION_320X240 | RESOLUTION_640X480 |
                         RESOLUTION_1280X720 | RESOLUTION_1600X1200 |
                         RESOLUTION_1920X1080 | RESOLUTION_2592X1944,
    .supportSpecialEffects = SPECIAL_BLUEISH | SPECIAL_REDISH | SPECIAL_BW |
                             SPECIAL_SEPIA | SPECIAL_NEGATIVE |
                             SPECIAL_GREENISH | SPECIAL_OVEREXPOSURE |
                             SPECIAL_SOLARIZE,
...
uint8_t ov3640GainValue[] = {0x00, 0x10, 0x18, 0x30, 0x34, 0x38, 0x3b, 0x3f,
...
CamStatus cameraBegin(ArducamCamera *MyCamera) {
  cameraInit(MyCamera);
  writeReg(MyCamera, CAM_REG_SENSOR_RESET,
           CAM_SENSOR_RESET_ENABLE); // reset cpld and camera
  ...
  MyCamera->currentPixelFormat =
      cameraDefaultInfo[MyCamera->cameraId].cameraDefaultFormat;
  MyCamera->currentPictureMode =
      cameraDefaultInfo[MyCamera->cameraId].cameraDefaultResolution;
  writeReg(MyCamera, CAM_REG_DEBUG_DEVICE_ADDRESS,
           CameraType[MyCamera->cameraId].deviceAddress);
  waitI2cIdle(MyCamera);
...
CamStatus cameraTakePicture(ArducamCamera *MyCamera, CAM_IMAGE_MODE mode,
                            CAM_IMAGE_PIX_FMT pixel_format) {
  if ((MyCamera->currentPixelFormat != pixel_format) ||
    ...
    waitI2cIdle(MyCamera);                            // Wait I2c Idle
    writeReg(MyCamera, CAM_REG_CAPTURE_RESOLUTION,
             CAM_SET_CAPTURE_MODE | mode); // set resolution
    waitI2cIdle(MyCamera);                 // Wait I2c Idle
  }
  setCapture(MyCamera);
  return CAM_ERR_SUCCESS;
...
CamStatus cameratakeMultiPictures(ArducamCamera *MyCamera, CAM_IMAGE_MODE mode,
...
void cameraRegisterCallback(ArducamCamera *MyCamera, BUFFER_CALLBACK function,
                            uint8_t size) {
...
CamStatus cameraSetAutoWhiteBalanceMode(ArducamCamera *MyCamera,
...
CamStatus cameraSetAutoWhiteBalance(ArducamCamera *MyCamera, uint8_t val) {
  ...
  writeReg(MyCamera, CAM_REG_EXPOSURE_GAIN_WHILEBALANCE_CONTROL,
...
CamStatus cameraSetISOSensitivity(ArducamCamera *MyCamera, int iso_sense) {
  ...
  writeReg(MyCamera, CAM_REG_MANUAL_GAIN_BIT_9_8,
...
CamStatus cameraSetAbsoluteExposure(ArducamCamera *MyCamera,
                                    uint32_t exposure_time) {
  writeReg(
      MyCamera, CAM_REG_MANUAL_EXPOSURE_BIT_19_16,
      (uint8_t)((exposure_time >> 16) & 0xff)); // set exposure output [19:16]
  waitI2cIdle(MyCamera);
  writeReg(
      MyCamera, CAM_REG_MANUAL_EXPOSURE_BIT_15_8,
      (uint8_t)((exposure_time >> 8) & 0xff)); // set exposure output [15:8]
  waitI2cIdle(MyCamera);
  writeReg(MyCamera, CAM_REG_MANUAL_EXPOSURE_BIT_7_0,
           (uint8_t)(exposure_time & 0xff)); // set exposure output [7:0]
  waitI2cIdle(MyCamera);
  return CAM_ERR_SUCCESS;
}
...
CamStatus cameraSetSaturation(ArducamCamera *MyCamera,
...
CamStatus cameraSetEV(ArducamCamera *MyCamera, CAM_EV_LEVEL level) {
  writeReg(MyCamera, CAM_REG_EV_CONTROL,
           level);       // set Exposure  Compensation Level
  waitI2cIdle(MyCamera); // Wait I2c Idle
  return CAM_ERR_SUCCESS;
}
...
CamStatus cameraSetSharpness(ArducamCamera *MyCamera,
...
CamStatus cameraSetBrightness(ArducamCamera *MyCamera,
...
void cameraWaitI2cIdle(ArducamCamera *MyCamera) {
  while ((readReg(MyCamera, CAM_REG_SENSOR_STATE) & 0X03) !=
...
CamStatus takePicture(ArducamCamera *MyCamera, CAM_IMAGE_MODE mode,
                      CAM_IMAGE_PIX_FMT pixel_format) {
  return MyCamera->arducamCameraOp->takePicture(MyCamera, mode, pixel_format);
}

CamStatus takeMultiPictures(ArducamCamera *MyCamera, CAM_IMAGE_MODE mode,
                            CAM_IMAGE_PIX_FMT pixel_format, uint8_t num) {
  return MyCamera->arducamCameraOp->takeMultiPictures(MyCamera, mode,
                                                      pixel_format, num);
}
...
CamStatus setAutoWhiteBalanceMode(ArducamCamera *MyCamera,
                                  CAM_WHITE_BALANCE mode) {
...
void registerCallback(ArducamCamera *MyCamera, BUFFER_CALLBACK function,
                      uint8_t blockSize) {
